  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 20px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <form onSubmit={handleSearch} style={{ display: 'flex', gap: 12, marginBottom: 40 }}>
        ...
        <button type="submit" disabled={loading} style={{ padding: '12px 24px', fontSize: 16, backgroundColor: '#011736', color: '#fff', border: 'none', borderRadius: 8, cursor: loading ? 'wait' : 'pointer', fontWeight: 600 }}>
      ...
      {results.length > 0 && (
        <div>
          <p style={{ color: '#666', marginBottom: 16 }}>{results.length} results</p>
          {results.map((r) => (
